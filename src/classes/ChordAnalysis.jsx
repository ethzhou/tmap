...
export default class ChordAnalysis {
  ...
  static fromString(strRepresentation) {
    const groups =
      strRepresentation?.match(REGEXP_ChordAnalysis)?.groups || undefined;

    return groups ? new ChordAnalysis(groups) : undefined;
  }

  toString() {
    return `${this.accidental ?? ""}${this.roman}${this.arabic ?? ""}${
      this.secondary ?? ""
    }`;
  }
  ...
  inversion() {
    return ["42"].includes(this.arabic)
      ? 3
      : ["43", "64"].includes(this.arabic)
      ? 2
      : ["65", "6", "63"].includes(this.arabic)
      ? 1
      : ["7", "53", "", undefined].includes(this.arabic)
      ? 0
